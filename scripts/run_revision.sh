#!/usr/bin/env bash
# Conservative job queue for the SNCS-D-26-07200 revision campaign.
#
...
# E1 first: the critical path. Seeds fixed in advance: 42 43 44 45 46.
for s in 42 43 44 45 46; do train corr_lr5_s$s 5e-6 $s; done
for s in 42 43 44 45 46; do train corr_lr75_s$s 7.5e-6 $s; done
for s in 42 43 44 45 46; do train corr_lr10_s$s 1e-5 $s; done
# E2, E4, E3-8q (already complete on this machine; kept so the queue is self-describing)
for ds in $DSETS; do t=$(ds_tag $ds)
  add "cmp_pre_$t" "$OUT/compression_matched_precorrection_$t.done" "" \
    "$PY scripts/eval_compression_matched.py --datasets $ds --data-dir $DATA --num-qubits 6 --train-size 200 --test-size 400 --seeds $SEEDS_EVAL --noise-grid $GRID6 --extra-models rep_lr5_s43,rep_lr75_s43,rep_lr10_s43,sft_warmup --cache-dirs results/replicate_circuits --output $OUT/compression_matched_precorrection_$t.json && touch $OUT/compression_matched_precorrection_$t.done"
  add "csweep_$t" "$OUT/c_sweep_$t.done" "" \
    "$PY scripts/eval_fusion_full.py --datasets $ds --data-dir $DATA --num-qubits 6 --train-size 200 --test-size 400 --seeds $SEEDS_EVAL --noise-grid 0.0,0.002,0.005,0.01 --no-ablation --c-values 0.1,1,10,100 --resume --output $OUT/c_sweep_$t.json && touch $OUT/c_sweep_$t.done"
done
for ds in IoT_Original_Distribution.csv UNSW_2018_IoT_Botnet_Final_10_Best.csv; do t=$(ds_tag $ds)
  add "real8q_$t" "$OUT/fusion_realistic_8q_$t.done" "" \
    "$PY scripts/eval_fusion_full.py --datasets $ds --data-dir $DATA --num-qubits 8 --train-size 200 --test-size 400 --seeds $SEEDS_EVAL --noise-pairs $PAIRS_REAL --no-ablation --resume --output $OUT/fusion_realistic_8q_$t.json && touch $OUT/fusion_realistic_8q_$t.done"
done
# Post-training pipeline, then the long ten-qubit runs last.
...
add emit_corr "$OUT/emit_corr.done" "corr_*" \
  "$PY scripts/emit_replicate_circuits.py --models $CORR_MODELS --base $BASE --num-qubits 6 --cache-dir $OUT/circuits --output $OUT/structure.json && touch $OUT/emit_corr.done"
add audit_corr "$OUT/audit_corr.done" "emit_corr" \
  "$PY scripts/audit_effective_params.py --circuits $OUT/circuits --output $OUT/effective_params.json && touch $OUT/audit_corr.done"
for ds in $DSETS; do t=$(ds_tag $ds)
  add "cmp_corr_$t" "$OUT/compression_matched_corrected_$t.done" "audit_corr" \
    "$PY scripts/eval_compression_matched.py --datasets $ds --data-dir $DATA --num-qubits 6 --train-size 200 --test-size 400 --seeds $SEEDS_EVAL --noise-grid $GRID6 --extra-models ${CORR_NAMES}rep_lr5_s43,rep_lr75_s43,rep_lr10_s43,sft_warmup --cache-dirs $OUT/circuits,results/replicate_circuits --output $OUT/compression_matched_corrected_$t.json && touch $OUT/compression_matched_corrected_$t.done"
done
add rescore_corr "$OUT/rescore_corr.done" "emit_corr" \
  "$PY scripts/rescore_reward_loop.py --circuits $OUT/circuits --models ${CORR_NAMES%,} --seeds 42,43,44,45,46 --train-size 48 --test-size 24 --util-metric mcc --util-reference clean_source --require-effective --output $OUT/rescoring_corrected_circuits_corrected_objective.json && $PY scripts/rescore_reward_loop.py --circuits $OUT/circuits --models ${CORR_NAMES%,} --seeds 42,43,44,45,46 --train-size 16 --test-size 8 --util-metric accuracy --util-reference noisy_source --output $OUT/rescoring_corrected_circuits_original_objective.json && touch $OUT/rescore_corr.done"
for ds in IoT_Original_Distribution.csv UNSW_2018_IoT_Botnet_Final_10_Best.csv; do t=$(ds_tag $ds)
  add "real10q_$t" "$OUT/fusion_realistic_10q_$t.done" "" \
    "$PY scripts/eval_fusion_full.py --datasets $ds --data-dir $DATA --num-qubits 10 --train-size 200 --test-size 400 --seeds $SEEDS_EVAL --noise-pairs $PAIRS_REAL --no-ablation --resume --output $OUT/fusion_realistic_10q_$t.json && touch $OUT/fusion_realistic_10q_$t.done"
done
